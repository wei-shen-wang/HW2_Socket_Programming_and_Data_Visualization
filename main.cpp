...
#else
    static constexpr size_t REMOTE_PORT = 1200; // standard HTTP port
#endif // MBED_CONF_APP_USE_TLS_SOCKET
...
public:
    ...
    void run()
    {
        ...
        //    return;
        //}
        //don't want to upload my real ip so use X.X.X.X instead
        //ip is hardcoded
        ...
        while (sample_num<100){
            ...
            char acc_json[1024];
            nsapi_size_t len = sprintf(acc_json ,"{\"x\":%d,\"y\":%d,\"z\":%d,\"s\":%d}",x, y, z, sample_num);
            nsapi_size_or_error_t response = _socket.send(acc_json,len);
            if (0 >= response){
                printf("Error seding: %dn", response);
            }
